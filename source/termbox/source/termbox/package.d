 ...
 * position.
 */
extern (C) void tb_put_cell(int x, int y, Cell* cell);
/// ditto
 ...
 */
extern (C) int tb_peek_event(Event* event, int timeout);

/** Wait for an event forever and fill the 'event' structure with it, when the
 * event is available. Returns the type of the event (one of TB_EVENT_*
 * constants) or -1 if there was an error.
 */
extern (C) int tb_poll_event(Event* event);

/* Utility utf8 functions. */
enum TB_EOF = -1;
extern (C) int tb_utf8_char_length(char c);
/// ditto
extern (C) int tb_utf8_char_to_unicode(uint* out_, const char* c);
/// ditto
extern (C) int tb_utf8_unicode_to_char(char* out_, uint c);